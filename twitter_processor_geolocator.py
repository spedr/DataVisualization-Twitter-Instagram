...
for idx, tweet in enumerate(items):
    if 'deals' in items[idx]['text'].lower():
        ...
        if '/' not in items[idx]['timezone']:
            if 'Pacific' in items[idx]['timezone']:
                items[idx]['timezone'] = 'US/Pacific'
                localize_time(items[idx]['created_at'], items[idx]['timezone'])
            elif 'Central' in items[idx]['timezone']:
                items[idx]['timezone'] = 'US/Central'
                localize_time(items[idx]['created_at'], items[idx]['timezone'])
            elif 'Eastern' in items[idx]['timezone']:
                items[idx]['timezone'] = 'US/Eastern'
                localize_time(items[idx]['created_at'], items[idx]['timezone'])
            elif 'Mountain' in items[idx]['timezone']:
                items[idx]['timezone'] = 'US/Mountain'
                localize_time(items[idx]['created_at'], items[idx]['timezone'])
            elif 'Atlantic' and 'Canada' in items[idx]['timezone']:
                items[idx]['timezone'] = 'Canada/Atlantic'
                localize_time(items[idx]['created_at'], items[idx]['timezone'])
            elif 'UTC' in items[idx]['timezone']:
                pass
            elif 'Hawaii' in items[idx]['timezone']:
                items[idx]['timezone'] = 'US/Hawaii'
                localize_time(items[idx]['created_at'], items[idx]['timezone'])
            elif 'Fiji' or 'Marshall Is.' in items[idx]['timezone']:
                items[idx]['timezone'] = 'Pacific/Fiji'
                localize_time(items[idx]['created_at'], items[idx]['timezone'])
            else:
                ...
                if found_timezone == False:
                    ##retry every 10 seconds in case Nominatim fails##
                    ...
                    def nominatim_attempt():
                        ...
                        if geolocator.geocode(items[idx]['timezone']) != None:
                            location = geolocator.geocode(items[idx]['timezone'])
                            tz = tzwhere.tzwhere(forceTZ=True)
                            current_location =  tz.tzNameAt(location.latitude, location.longitude)
                            timezone = pytz.timezone(current_location)
                            current_date = datetime.strptime(items[idx]['created_at'],'%Y-%m-%dT%H:%M:%SZ')
                            localized_time = pytz.utc.localize(current_date, is_dst=None).astimezone(timezone)
                            time_list.append(localized_time)
            ...
            if items[idx]['timezone'] not in twitter_default_times_list:
                twitter_default_times_list.append(items[idx]['timezone'])
            localize_time(items[idx]['created_at'], items[idx]['timezone'])

print twitter_default_times_list
